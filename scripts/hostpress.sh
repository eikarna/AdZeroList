...
# Additional filter cleaning for uBlock Origin, because ABP doesnt support
grep -vE '(:has\(|:is\(|\#@|##|#\$|^\|\|.*\$|^\|\|.*\+\js\()' "$TEMP_ADBLOCK_PROCESSED" > "$TEMP_ADBLOCK_PROCESSED.tmp" || true
mv "$TEMP_ADBLOCK_PROCESSED.tmp" "$TEMP_ADBLOCK_PROCESSED"
